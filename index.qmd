...
dating_app_data |>
  ...
  # geom_text(aes(label = paste0(app_usage_time_label*100,"%")), 
...
The density plot above shows the distribution of app usage time across different location types.
From the plot, we can see that metro users tend to have a higher concentration of longer app usage times compared to those from more remote areas.
This suggests that users in urban locations might spend more time on dating apps, which could possibly be due to greater social interaction or availability of matches.
